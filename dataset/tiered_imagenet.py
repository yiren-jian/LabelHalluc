...
class MetaTieredImageNet(TieredImageNet):
    ...
    def __init__(self, args, partition='train', train_transform=None, test_transform=None, fix_seed=True, return_numpy=True):
    ...
    def __getitem__(self, item):
        ...
        numpy_support_xs = support_xs
        numpy_query_xs = query_xs

        support_xs = torch.stack(list(map(lambda x: self.train_transform(x.squeeze()), support_xs)))
        query_xs = torch.stack(list(map(lambda x: self.test_transform(x.squeeze()), query_xs)))

        if self.return_numpy :
            return support_xs, support_ys, query_xs, query_ys, (numpy_support_xs, numpy_query_xs)
        else:
