...
def meta_test(net, trainloader1, trainloader2, testloader, use_logit=True, is_norm=True, classifier='LR', opt=None):
    ...
    for idx, data in tqdm(enumerate(testloader)):
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            time1 = time.time()
            train_one_epoch(epoch, trainloader2, numpy_support_xs, support_ys, query_xs, query_ys, learner, kd_criterion, optimizer, opt)
            # scheduler.step()
            time2 = time.time()
...
def train_one_epoch(epoch, train_loader, numpy_support_xs, support_ys, query_xs, query_ys, model, kd_criterion, optimizer, opt):
    """One epoch training"""
    ...
    """
    Ablation study on finetuning with support examples only
    """
    end = time.time()
    for idx, (input, target, item) in enumerate(train_loader):
        ...
        # ===================forward=====================
        output = model(support_xs, norm_feat=opt.norm_feat)  # end-to-end model with option to normalize the feature before model.classifier
        loss = nn.CrossEntropyLoss()(output, support_ys)

        losses2.update(loss.item(), input.size(0))
