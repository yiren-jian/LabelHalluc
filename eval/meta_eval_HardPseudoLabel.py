...
def meta_test(net, trainloader1, trainloader2, testloader, use_logit=True, is_norm=True, classifier='LR', opt=None):
    ...
    net_state_dict = net.state_dict()          # Starting point for each episode finetuning.
    ...
    for idx, data in tqdm(enumerate(testloader)):
        ...
        trainset_items = []      # storing index for each base set example
        trainset_labels = []     # storing hard label for each base set example
        trainset_pseudo_ys = []  # storing pseudo soft label (logits actually) for each base set example

        for _, (input, target, trainset_item) in enumerate(trainloader1):    # trainloader1 (with test transform) is used for pseudo labeling
            input = input.float()
            ...
            if is_norm: # by default is True, following RFS (ECCV 2020)
                input_feat = normalize(input_feat)

            input_feat = input_feat.detach().cpu().numpy()
            label = target.numpy() # hard label
            pseudo_y = clf.decision_function(input_feat) # This is logit, beofre softmax. We will use it with temperature T later in KD.

            trainset_items.append(trainset_item.numpy())
            trainset_labels.append(label)
            trainset_pseudo_ys.append(pseudo_y)

        trainset_items = np.concatenate(trainset_items, axis=0)          # dimension is (num_examples, )
        trainset_labels = np.concatenate(trainset_labels, axis=0)        # dimension is (num_examples, )
        trainset_pseudo_ys = np.concatenate(trainset_pseudo_ys, axis=0)  # dimension is (num_examples, num_classes=5), num_classes is equal to opt.n_ways
        ...
        opt.step_counter = 0
        opt.total_steps_in_each_episode = opt.epochs * len(trainloader2)
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            acc_new[epoch-1].append(metrics.accuracy_score(query_ys, query_ys_pred))
        ...
        for e in range(len(acc_new)):
            print('epochs: {:d}, acc2: {:.4f}, std2: {:.4f}'.format((e+1),
...
def train_one_epoch(epoch, train_loader, pseudo_ys, numpy_support_xs, support_ys, query_xs, query_ys, model, kd_criterion, optimizer, opt):
    """One epoch training"""
    ...
    """Ablation study for soft v.s. hard labels"""
    end = time.time()
    for idx, (input, target, item) in enumerate(train_loader):
        ...
        pseudo_y = pseudo_ys[item.numpy()] # get the pseudo label (logit) of current batch by indexing (item.numpy is indices)
        input = input.float()
        input = input.cuda()
        ...
        loss1 = nn.CrossEntropyLoss()(output[:opt.train_batch_size], pseudo_target)               # first half of mini-batch is base examples
        ...
        if opt.step_counter > 0 and opt.step_counter-1 == opt.early_stop_steps:
            return 0 # early stopping for 1-shot cases

    return query_accuracy  # query accuracy at the end of each epoch, but this is not used actually
